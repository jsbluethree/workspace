...
struct ISceneGraph{
	...
	virtual bool check_collision(float x, float y) { return check_collision(FloatRect(x, y, 0, 0)); }
	virtual bool check_collision(const Vector2f& v) { return check_collision(v.x, v.y); }
	virtual bool check_collision(ISceneNode* node) { return check_collision(node->get_rect()); }
