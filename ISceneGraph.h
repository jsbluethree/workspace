...
struct ISceneGraph{
	...
	virtual bool check_collision(ISceneNode* node) const { return check_collision(node->get_rect()); }
	...
	virtual std::unordered_set<ISceneNode*> get_collision(ISceneNode* node) const { return get_collision(node->get_rect()); }
};
